...
export default function Portfolio() {
  ...
  const summary = {
    ...
    total_profit_pct: Number(rawSummary.total_profit_pct ?? 0),
  }
  const isProfit = summary.total_profit >= 0
  const pieData = positions.map(p => ({ name: p.name, value: Number(p.value ?? 0) }))
        ...
        {positions.length === 0 ? (
          <div className="flex flex-col items-center gap-4 mt-24">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">从分析页面登记买入记录</p>
            </div>
          ...
          <>
            <div className={clsx(
              ...
              isProfit ? 'bg-gradient-to-br from-red-500 to-red-400' : 'bg-gradient-to-br from-teal-500 to-teal-400'
            )}>
              ...
              <div className="card">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    {positions.map((p, i) => {
                      const pct = summary.total_value > 0 ? (Number(p.value ?? 0) / summary.total_value * 100) : 0
                      return (
...
function PositionCard({ pos, onDelete, onTap }: {
  pos: PortfolioPosition; onDelete: () => void; onTap: () => void
}) {
  const profitPct = Number(pos.profit_pct ?? 0)
  const changePct = Number(pos.change_pct ?? 0)
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            <div className="text-sm font-semibold text-gray-800 num">¥{currentPrice.toFixed(2)}</div>
            <div className={clsx('text-xs num', todayUp ? 'text-up' : 'text-down')}>
              今日 {todayUp ? '+' : ''}{changePct.toFixed(2)}%
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: '买入价', val: `¥${buyPrice.toFixed(2)}` },
            { label: '持仓市值', val: `¥${value.toFixed(2)}` },
            { label: '总盈亏', val: `${isUp ? '+' : ''}${profitPct.toFixed(2)}%` },
          ].map(item => (
